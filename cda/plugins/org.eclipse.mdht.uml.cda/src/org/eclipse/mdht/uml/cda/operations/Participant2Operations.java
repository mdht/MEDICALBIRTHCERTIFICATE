...
public class Participant2Operations extends ParticipationOperations {
    ...
    DatatypesUtil.increment(context,"Participant2ContextControlCode","null");
		...
		if (!EOCL_ENV.get().createQuery(VALIDATE_CONTEXT_CONTROL_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get()).check(participant2)) {
			if (diagnostics != null) {
				diagnostics.add
					(new BasicDiagnostic
						(Diagnostic.ERROR,
